...
構成は [arc42](https://arc42.org/overview/) の抜粋。この規模で意味のある章だけを使っている。
...
| シェルを補助に使わない | 補完も probe も、編集中の行や生成物をシェルに渡さない | 品質目標1 |
| 画面を占有しない | 端末の代替画面バッファに入らない | 実行結果が通常のシェルと同じようにスクロールバックへ残る必要がある |
| 自動で起動する外部プロセスは読み取りのみ | `--help` / `man` / probe / `tmux capture-pane` | 品質目標1 |
...
| 実行前に利用者へ確認を求める | 候補5件ぶんの確認が出る。5秒で候補が並ぶ体験が壊れる |
