...
| cmdmuse はテキストを補完するのみ | 実行はしない。確定した行は `$SHELL -c` に渡す | 要件 |
...
| 利用者が実行したコマンドの出力を画面に残す | cmdmuse は端末の代替画面バッファ (vim や less が使うもの) に入らない。プロンプトも候補も、`$SHELL` が出力した内容も、通常の画面にそのまま書かれる | 要件 |
