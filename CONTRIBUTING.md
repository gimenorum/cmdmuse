...
なぜそうなっているかは [docs/DESIGN.md](docs/DESIGN.md) に書いてある。手を入れる前に読むこと。

**probe はシェルを介さない。** `exec` に argv を直接渡し、コマンド名とサブコマンドを
許可リストで絞る。新しいコマンドを許可リストに足すときは、それが**副作用を持たない**ことを
確認すること。`internal/probe/probe_test.go` の危険入力テストは消さない。
